...
pub fn convert_hopcounts_to_ticks(
    hopcounts: &Vec<usize>,
    tps: f64,
    hop_size: usize,
    frame_rate_hz: u32,
    ...
    let mut tickdiff: Vec<usize> = diff
        .iter()
        .map(|c| ((*c * hop_size) as f64 / frame_rate_hz as f64 * tps + 0.5) as usize)
        .collect();
...
pub fn export_notes(notes: &Vec<Vec<crate::note::Note>>, timestamps: &Vec<usize>, tps: f64) {
    assert_eq!(
        notes.len(),
        timestamps.len(),
        "Amount of note vecs should be the amount of timestamps!"
    );

    let layer_counts_by_instrid: Vec<usize> = (0..note::INSTRUMENT_COUNT)
        .map(|instr_id| {
            notes
                .iter()
                .max_by_key(|tick| {
                    tick.iter()
                        .filter(|note| note.instrument_id == instr_id)
                        .count()
                })
                .unwrap()
                .iter()
    ...
    for i in 0..notes.len() {
        ...
        for j in 0..notes[i].len() {
            let note = &notes[i][j];
            let vol =
                (std::cmp::min(std::cmp::max((note.volume.abs() * 100.0) as i32, 0), 100)) as i8;

            //if vol < 10 { continue; }

            if current_instrid != note.instrument_id {
                current_instrid = note.instrument_id;
                current_instrid_beginning = (0..note.instrument_id)
                    .map(|i| layer_counts_by_instrid[i])
                    .sum();
                instrid_count = 0;
            }

            noteblocks.layers[current_instrid_beginning + instrid_count]
                .notes
                .insert(
                    timestamps[i] as i16,
                    Note::new(

...
pub fn create_spectrum(
    ...
    fft_size: usize,
    hop_size: usize,
    hop_count: isize, /*<0 if full conversion*/
    ...
    padded_samples.resize(
        max(samples.len() as isize, hop_size as isize * hop_count) as usize + fft_size,
        0.0,
    ); // so that empty samples is converted well as well

    let last_sample = if hop_count < 0 {
        samples.len()
    } else {
        hop_size * hop_count as usize
    };
...
pub fn waveform_to_spectrogram(
    wf: &Waveform,
    fft_size: usize,
    hop_size: usize,
) -> note::Spectrogram {
    let spectrogram = create_spectrum(
...
pub fn waveform_to_spectrogram_countlimited(
    wf: &Waveform,
    fft_size: usize,
    hop_size: usize,
    hop_count: usize,
...
pub fn get_interesting_hopcounts(spectrogram: &note::Spectrogram) -> Vec<usize> {
    ...
    for i in 0..(sumvec.len() - 1) {
        if (i < 1 || sumvec[i-1] * 1.2/* TODO magic number */ < sumvec[i])
            && (i >= sumvec.len() - 1
                || i < 1
                || sumvec[i] - sumvec[i - 1] > sumvec[i + 1] - sumvec[i])
...
pub fn subtract_2d_vecs(one: &[Vec<f32>], other: &[Vec<f32>]) -> Vec<Vec<f32>> {
    ...
    for x in 0..bigger_width {
        for y in 0..height {
            let one_val = match one.get(x) {
                Some(v) => v[y],
                None => 0.0,
            };
            let other_val = match other.get(x) {
                Some(v) => v[y],
                None => 0.0,
            };
...
pub fn change_pitch(wf: &Waveform, multiplier: f32) -> Waveform {
    ...
    let new_wf = wf
        .resample((original_hz as f32 / multiplier) as u32)
        .unwrap();

    //println!("Through {:?}", new_wf);

    let even_newer_wf = Waveform::from_interleaved_samples(
        original_hz,
        new_wf.num_channels(),
        new_wf.to_interleaved_samples(),
    );
